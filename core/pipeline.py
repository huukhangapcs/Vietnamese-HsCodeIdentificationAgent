    ...
    def classify(self, item_description: str, extracted_features: dict = None, max_revisions: int = 2, stream_callback=None, input_callback=None):
        ...
        if candidate_pool:
            ...
            if judge_res.get("status") == "SUCCESS":
                ...
                if is_valid:
                    chosen_cand = next((c for c in candidate_pool if c["hs_code"] == chosen_code), None)
                    draft_result_for_auditor = {
                        "hs_code": chosen_code,
                        "description": chosen_cand.get("description", "") if chosen_cand else "Unknown",
                        "reasoning": judge_res.get("reasoning", "")
                    }
                    audit_res = self.auditor.audit(item_description, draft_result_for_auditor)
